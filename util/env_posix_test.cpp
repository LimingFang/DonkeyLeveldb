...
//
// DonkeyLeveldb project modified some part of this file.
...
TEST_F(EnvPosixTest, TestOpenOnRead) {
  ...
  ASSERT_TRUE(env_->RemoveFile(test_file).ok());
}
...
TEST_F(EnvPosixTest, TestCloseOnExecSequentialFile) {
  ...
  ASSERT_TRUE(WriteStringToFile(env_, "0123456789", file_path).ok());

  leveldb::SequentialFile* file = nullptr;
  ASSERT_TRUE(env_->NewSequentialFile(file_path, &file).ok());
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;

  ASSERT_TRUE(env_->RemoveFile(file_path).ok());
}
...
TEST_F(EnvPosixTest, TestCloseOnExecRandomAccessFile) {
  ...
  for (int i = 0; i < kMMapLimit; i++) {
    ASSERT_TRUE(env_->NewRandomAccessFile(file_path, &mmapped_files[i]).ok());
  }

  leveldb::RandomAccessFile* file = nullptr;
  ASSERT_TRUE(env_->NewRandomAccessFile(file_path, &file).ok());
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecWritableFile) {
  ...
  ASSERT_TRUE(env_->NewWritableFile(file_path, &file).ok());
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecLockFile) {
  ...
  ASSERT_TRUE(env_->LockFile(file_path, &lock).ok());
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
...
TEST_F(EnvPosixTest, TestCloseOnExecLogger) {
  ...
  ASSERT_TRUE(env_->NewLogger(file_path, &file).ok());
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
